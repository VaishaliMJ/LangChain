...
def main():
    ...
    stateName=st.text_input("Enter state name")
    
    if st.button("Find Capital and its current weather"):
        request={"input" :f"Find the captial of {stateName},then find it's current weather condition"}
        response=agentExecutor.invoke(request) 
